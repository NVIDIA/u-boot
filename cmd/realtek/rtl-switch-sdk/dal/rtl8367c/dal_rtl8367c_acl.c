 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_cfg_delAll(rtksw_uint32 unit)
{
    rtksw_uint32            i;
    rtksw_api_ret_t     ret;
    ...
    {
        if((ret = rtl8367c_setAsicAclActCtrl(unit, i, FILTER_ENACT_INIT_MASK))!= RT_ERR_OK)
            return ret;
        if((ret = rtl8367c_setAsicAclNot(unit, i, RTKSW_DISABLED)) != RT_ERR_OK )
            return ret;
    }

    return rtl8367c_setAsicRegBit(unit, RTL8367C_REG_ACL_RESET_CFG, RTL8367C_ACL_RESET_CFG_OFFSET, TRUE);
}
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_init(rtksw_uint32 unit)
{
    ...
    if ((ret = dal_rtl8367c_filter_igrAcl_cfg_delAll(unit)) != RT_ERR_OK)
        return ret;
    ...
    for(i = 0; i < RTL8367C_ACLTEMPLATENO; i++)
    {
        ...
        if ((ret = rtl8367c_setAsicAclTemplate(unit, i, &aclTemp)) != RT_ERR_OK)
            return ret;
    }

    for(i = 0; i < RTL8367C_FIELDSEL_FORMAT_NUMBER; i++)
    {
        if ((ret = rtl8367c_setAsicFieldSelector(unit, i, field_selector[i][0], field_selector[i][1])) != RT_ERR_OK)
            return ret;
    }

    RTKSW_SCAN_ALL_PHY_PORTMASK(unit, i)
    {
        if ((ret = rtl8367c_setAsicAcl(unit, i, TRUE)) != RT_ERR_OK)
            return ret;

        if ((ret = rtl8367c_setAsicAclUnmatchedPermit(unit, i, TRUE)) != RT_ERR_OK)
            return ret;
    }
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_field_add(rtksw_uint32 unit, rtksw_filter_cfg_t* pFilter_cfg, rtksw_filter_field_t* pFilter_field)
{
    rtksw_uint32 i;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_cfg_add(rtksw_uint32 unit, rtksw_filter_id_t filter_id, rtksw_filter_cfg_t* pFilter_cfg, rtksw_filter_action_t* pFilter_action, rtksw_filter_number_t *ruleNum)
{
    ...
    for(i = 0; i < RTL8367C_ACLTEMPLATENO; i++)
    {
        osal_memset(&aclRule[i], 0, sizeof(rtl8367c_aclrule));
    ...
    *ruleNum = noRulesAdd;
    ...
    osal_memset(&aclAct, 0, sizeof(rtl8367c_acl_act_t));
    aclActCtrl = 0;
    for(actType = 0; actType < FILTER_ENACT_END; actType ++)
    {
        if(pFilter_action->actEnable[actType])
        {
            ...
            case FILTER_ENACT_CVLAN_INGRESS:
                ...
                if((retVal = dal_rtl8367c_vlan_checkAndCreateMbr(unit, pFilter_action->filterCvlanVid, &vidx)) != RT_ERR_OK)
                {
                    return retVal;
                }
            ...
            case FILTER_ENACT_SVLAN_EGRESS:
                ...
                if((retVal = dal_rtl8367c_svlan_checkAndCreateMbr(unit, pFilter_action->filterSvlanVid, &svidx)) != RT_ERR_OK)
                    return retVal;
            ...
            case FILTER_ENACT_COPY_CPU:
                if((retVal = rtl8367c_getAsicCputagTrapPort(unit, &cpuPort)) != RT_ERR_OK)
                    return retVal;
    ...
    {
        if((retVal = rtl8367c_getAsicAclRule(unit, i, &tempRule)) != RT_ERR_OK )
            return retVal;
    ...
    for(i = 0; i < RTL8367C_ACLTEMPLATENO; i++)
    {
        if(aclRule[i].valid == TRUE)
        {
            ...
            if((retVal = rtl8367c_setAsicAclActCtrl(unit, filter_id + ruleId, aclActCtrl)) != RT_ERR_OK )
                return retVal;
            /* write ACL action */
            if((retVal = rtl8367c_setAsicAclAct(unit, filter_id + ruleId, &aclAct)) != RT_ERR_OK )
                return retVal;

            /* write ACL not */
            if((retVal = rtl8367c_setAsicAclNot(unit, filter_id + ruleId, pFilter_cfg->invert)) != RT_ERR_OK )
                return retVal;
            /* write ACL rule */
            if((retVal = rtl8367c_setAsicAclRule(unit, filter_id + ruleId, &aclRule[i])) != RT_ERR_OK )
                return retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_cfg_del(rtksw_uint32 unit, rtksw_filter_id_t filter_id)
{
    ...
    osal_memset(&initRule, 0, sizeof(rtl8367c_aclrule));
    osal_memset(&initAct, 0, sizeof(rtl8367c_acl_act_t));

    if((ret = rtl8367c_setAsicAclRule(unit, filter_id, &initRule)) != RT_ERR_OK)
        return ret;
    if((ret = rtl8367c_setAsicAclActCtrl(unit, filter_id, FILTER_ENACT_INIT_MASK))!= RT_ERR_OK)
        return ret;
    if((ret = rtl8367c_setAsicAclAct(unit, filter_id, &initAct)) != RT_ERR_OK)
        return ret;
    if((ret = rtl8367c_setAsicAclNot(unit, filter_id, RTKSW_DISABLED)) != RT_ERR_OK )
        return ret;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_cfg_get(rtksw_uint32 unit, rtksw_filter_id_t filter_id, rtksw_filter_cfg_raw_t *pFilter_cfg, rtksw_filter_action_t *pAction)
{
    rtksw_api_ret_t               retVal;
    rtksw_uint32                  i, tmp;
    ...
    if ((retVal = rtl8367c_getAsicAclRule(unit, filter_id, &aclRule)) != RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_getAsicAclNot(unit, filter_id, &tmp))!= RT_ERR_OK)
        return retVal;
    ...
    if ((retVal = rtl8367c_getAsicAclActCtrl(unit, filter_id, &tmp))!= RT_ERR_OK)
        return retVal;

    if ((retVal = rtl8367c_getAsicAclAct(unit, filter_id, &aclAct)) != RT_ERR_OK)
        return retVal;
    ...
    if(tmp & FILTER_ENACT_SVLAN_MASK)
    {
        ...
        {
            if((retVal = rtl8367c_getAsicSvlanMemberConfiguration(unit, aclAct.svidx_sact, &svlan_cfg)) != RT_ERR_OK)
                return retVal;
    ...
    if(tmp & FILTER_ENACT_CVLAN_MASK)
    {
            ...
            FILTER_ENACT_CACTEXT_BOTHVLANTAG == aclAct.cact_ext )
        {
            ...
            {
                if((retVal = rtl8367c_getAsicVlanMemberConfig(unit, aclAct.cvidx_cact, &vlanMC)) != RT_ERR_OK)
                    return retVal;
    ...
    if ((retVal = rtl8367c_getAsicAclTemplate(unit, aclRule.data_bits.type, &type))!= RT_ERR_OK)
        return retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_unmatchAction_set(rtksw_uint32 unit, rtksw_port_t port, rtksw_filter_unmatch_action_t action)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAclUnmatchedPermit(unit, rtksw_switch_port_L2P_get(unit, port), action)) != RT_ERR_OK)
       return ret;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_unmatchAction_get(rtksw_uint32 unit, rtksw_port_t port, rtksw_filter_unmatch_action_t* pAction)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAclUnmatchedPermit(unit, rtksw_switch_port_L2P_get(unit, port), pAction)) != RT_ERR_OK)
       return ret;
 ...
 *      This function gets action of packets when no ACL configruation matches.
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_state_set(rtksw_uint32 unit, rtksw_port_t port, rtksw_filter_state_t state)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAcl(unit, rtksw_switch_port_L2P_get(unit, port), state)) != RT_ERR_OK)
       return ret;
 ...
 *      This function gets action of packets when no ACL configruation matches.
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_state_get(rtksw_uint32 unit, rtksw_port_t port, rtksw_filter_state_t* pState)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAcl(unit, rtksw_switch_port_L2P_get(unit, port), pState)) != RT_ERR_OK)
       return ret;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_template_set(rtksw_uint32 unit, rtksw_filter_template_t *aclTemplate)
{
    rtksw_api_ret_t retVal;
    ...
    if((retVal = rtl8367c_setAsicAclTemplate(unit, aclTemplate->index, &aclType)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_template_get(rtksw_uint32 unit, rtksw_filter_template_t *aclTemplate)
{
    rtksw_api_ret_t ret;
    rtksw_uint32 idxField;
    rtl8367c_acltemplate_t aclType;
   ...
   if((ret = rtl8367c_getAsicAclTemplate(unit, aclTemplate->index, &aclType)) != RT_ERR_OK)
       return ret;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_field_sel_set(rtksw_uint32 unit, rtksw_uint32 index, rtksw_field_sel_t format, rtksw_uint32 offset)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicFieldSelector(unit, index, (rtksw_uint32)format, offset)) != RT_ERR_OK)
       return ret;
 ...
 *      None.
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAcl_field_sel_get(rtksw_uint32 unit, rtksw_uint32 index, rtksw_field_sel_t *pFormat, rtksw_uint32 *pOffset)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicFieldSelector(unit, index, pFormat, pOffset)) != RT_ERR_OK)
       return ret;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_iprange_set(rtksw_uint32 unit, rtksw_uint32 index, rtksw_filter_iprange_t type, ipaddr_t upperIp, ipaddr_t lowerIp)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAclIpRange(unit, index, type, upperIp, lowerIp)) != RT_ERR_OK)
       return ret;
 ...
 *      None.
 */
rtksw_api_ret_t dal_rtl8367c_filter_iprange_get(rtksw_uint32 unit, rtksw_uint32 index, rtksw_filter_iprange_t *pType, ipaddr_t *pUpperIp, ipaddr_t *pLowerIp)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAclIpRange(unit, index, pType, pUpperIp, pLowerIp)) != RT_ERR_OK)
       return ret;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_vidrange_set(rtksw_uint32 unit, rtksw_uint32 index, rtksw_filter_vidrange_t type, rtksw_uint32 upperVid, rtksw_uint32 lowerVid)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAclVidRange(unit, index, type, upperVid, lowerVid)) != RT_ERR_OK)
       return ret;
 ...
 *      None.
 */
rtksw_api_ret_t dal_rtl8367c_filter_vidrange_get(rtksw_uint32 unit, rtksw_uint32 index, rtksw_filter_vidrange_t *pType, rtksw_uint32 *pUpperVid, rtksw_uint32 *pLowerVid)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAclVidRange(unit, index, pType, pUpperVid, pLowerVid)) != RT_ERR_OK)
       return ret;
 ...
 */
rtksw_api_ret_t dal_rtl8367c_filter_portrange_set(rtksw_uint32 unit, rtksw_uint32 index, rtksw_filter_portrange_t type, rtksw_uint32 upperPort, rtksw_uint32 lowerPort)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_setAsicAclPortRange(unit, index, type, upperPort, lowerPort)) != RT_ERR_OK)
       return ret;
 ...
 *      None.
 */
rtksw_api_ret_t dal_rtl8367c_filter_portrange_get(rtksw_uint32 unit, rtksw_uint32 index, rtksw_filter_portrange_t *pType, rtksw_uint32 *pUpperPort, rtksw_uint32 *pLowerPort)
{
    rtksw_api_ret_t ret;
    ...
    if((ret = rtl8367c_getAsicAclPortRange(unit, index, pType, pUpperPort, pLowerPort)) != RT_ERR_OK)
       return ret;
 ...
 *      none
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAclPolarity_set(rtksw_uint32 unit, rtksw_uint32 polarity)
{
    /* Check initialization state */
    RTKSW_CHK_INIT_STATE(unit);
    ...
    return rtl8367c_setAsicAclGpioPolarity(unit, polarity);
}
 ...
 *      none
 */
rtksw_api_ret_t dal_rtl8367c_filter_igrAclPolarity_get(rtksw_uint32 unit, rtksw_uint32* pPolarity)
{
    /* Check initialization state */
    RTKSW_CHK_INIT_STATE(unit);
    ...
    return rtl8367c_getAsicAclGpioPolarity(unit, pPolarity);
}
