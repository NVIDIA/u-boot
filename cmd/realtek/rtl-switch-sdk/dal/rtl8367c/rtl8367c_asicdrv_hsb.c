 ...
 */
ret_t rtl8367c_setAsicFieldSelector(rtksw_uint32 unit, rtksw_uint32 index, rtksw_uint32 format, rtksw_uint32 offset)
{
    ...
    return rtl8367c_setAsicReg(unit, RTL8367C_FIELD_SELECTOR_REG(index), regData);
}
/* Function Name:
 ...
 *      None
 */
ret_t rtl8367c_getAsicFieldSelector(rtksw_uint32 unit, rtksw_uint32 index, rtksw_uint32* pFormat, rtksw_uint32* pOffset)
{
    ret_t retVal;
    rtksw_uint32 regData;

    retVal = rtl8367c_getAsicReg(unit, RTL8367C_FIELD_SELECTOR_REG(index), &regData);
    if(retVal != RT_ERR_OK)
        return retVal;
